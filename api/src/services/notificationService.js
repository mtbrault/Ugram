...
const create = async (userId, text = "empty") => {
	let notification = new Notification({ userId: userId._id, text });
...
const getById = async (id) => {
	...
	if (!notification) throw new Error(`notification with id ${id} doesn't exist`);
	return notification;
};
...
const markAllAsRead = async (user) => {
	const notification = await Notification.updateMany(
		{ userId: user._id || user, isRead: false },
		{ $set: { isRead: true } },
	);
	return notification;
};
