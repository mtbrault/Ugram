...
const getAll = async (req, res, next) => {
	...
	if (!last)
		chunk.next = `/user/search?page=${req.page + 1}&limit=${req.limit}${
			req.query.noself ? `&noself=${req.query.noself}` : ""
		}`;
...
const remove = async (req, res, next) => {
	const [err] = await to(userService.remove(req.user));
	if (err) return rerr(next, err);
...
const removeById = async (req, res, next) => {
	const [err] = await to(userService.remove(req.refUser));
