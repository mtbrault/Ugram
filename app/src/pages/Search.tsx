...
const Search: React.FC<SearchProps> = ({ history }) => {
  ...
  useEffect(() => {
    if (search.users_list.users.length === 0 && search.posts_desc.p)
  }, [search])
          ...
          <List
            ...
            header={(
              <h3 className="title-h1">
                Result of users&nbsp;
                <Tag>{search.users_list.users.length}</Tag>
              </h3>
            )}
            itemLayout="horizontal"
            dataSource={search.users_list.users}
            /*
                        loadMore={(search.next) ? loadMore : ''}
            */
